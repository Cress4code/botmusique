...
// setSession
    ...
    {
        $this->sendAction("type_on");
        $subscriber_id = $this->sender_id;
        $subscriber_data = (array)$this->getFaceBookUserData($subscriber_id);

        //$this->sendText(json_encode($subscriber_data));
        unset($subscriber_data['profile_pic']);
        Subscriber::insert_or_update($subscriber_id, $subscriber_data);
        $_SESSION['current_subscriber'] = $subscriber_data;
        $_SESSION['current_subscriber']['id'] = $subscriber_id;
    }
...
// run
    ...
    public function getFaceBookUserData($userID)
    {
        ...
        $url = "https://graph.facebook.com/v2.6/" . $userID . "?fields=first_name,last_name,profile_pic&access_token=" . $this->page_access_token;
